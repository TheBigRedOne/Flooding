...
class Nfd(Application):
    def __init__(self, node, logLevel='INFO', csSize=65536,
        ...
        if "tcp" not in conf_file["face_system"]:
            conf_file["face_system"]["tcp"] = {}
        conf_file["face_system"]["tcp"]["listen"] = "yes"
        conf_file["face_system"]["tcp"]["port"] = 6363
        conf_file["face_system"]["tcp"]["enable_v4"] = "yes"
        conf_file["face_system"]["tcp"]["enable_v6"] = "yes"

        # Create client configuration for host to ensure TCP configuration is consistent
