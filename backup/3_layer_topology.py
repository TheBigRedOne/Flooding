...
def dynamic_change(net):
    # Wait for 5 seconds before making changes
    time.sleep(5)
...
def run():
    ...
    # Start tcpdump to capture packets for 20 seconds on consumer and producer
    consumer_pcap = '/home/m26a1pershing/NDN/mini-ndn/examples/flooding/consumer.pcap'
    producer_pcap = '/home/m26a1pershing/NDN/mini-ndn/examples/flooding/producer.pcap'
    consumer.cmd(f'sudo tcpdump -i any -w {consumer_pcap} &')
    producer.cmd(f'sudo tcpdump -i any -w {producer_pcap} &')

    # Automatically send interests from consumer
    consumer.cmd('ndnping /example/producer > /tmp/ndnping.log &')
    ...
    # Stop tcpdump
    consumer.cmd('sudo pkill tcpdump')
    producer.cmd('sudo pkill tcpdump')
